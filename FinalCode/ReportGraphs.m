...
[rx, rx_cut, rx_cut_ref, testName] = loadNodeData_wav (directory, ref_node, 1,SAMPLERATE, N);
...
saveas(1,'..\Final Report\MEng_Project_Report_template\figures\90degPhasePlotPre','epsc')
...
xlabel('Sample Number')
legend('Node 1', 'Node 2', 'Node 3', 'Node 4', 'Node 5');
saveas(2,'..\Final Report\MEng_Project_Report_template\figures\90degPhasePlot','epsc')


[rx_sample_out, coeffs] = LoCorrectionLin_reportgraph(rx_sample,1,192000);

[phaseOffset] = getPhaseOffset (coeffs(:,2), array,DOA);
saveas(3,'..\Final Report\MEng_Project_Report_template\figures\90degPhasePlot_Lin_correction','epsc')
saveas(4,'..\Final Report\MEng_Project_Report_template\figures\90degPhasePlot_Lin_corrected','epsc')

[phaseCorrectedSymbols] = LoCorrection(rx_sample, coeffs(:,1),phaseOffset, 1,SAMPLERATE);

saveas(6,'..\Final Report\MEng_Project_Report_template\figures\90degPhasePlot_Lin_correctedFully','epsc')

[gainCorrectedSymbols] = gainCorr(phaseCorrectedSymbols, 1,ref_node);
 
saveas(7,'..\Final Report\MEng_Project_Report_template\figures\90degMagnitudeBeforeCorr','epsc')
saveas(8,'..\Final Report\MEng_Project_Report_template\figures\90degMagnitudecorrectedFully','epsc')
...
grid on;
title('MUSIC spectrum for a source from (90,0), showing calibration effects') 
xlabel('Azimuth (degrees)')
ylabel('dB')     
legend('Uncorrected','LO F', 'LO Freq and phase', 'LO Freq, Phase, and Gain')
hold off
saveas(9,'..\Final Report\MEng_Project_Report_template\figures\90degMUSICm1','epsc')
...
plot(Azarea,Z_LOphasegain);
grid on;
title('MUSIC spectrum for a source from (90,0), after Pilot Calibration') 
xlabel('Azimuth (degrees)')
ylabel('dB')     
legend('Uncorrected','LO Freq, Phase, and Gain')
hold off

saveas(10,'..\Final Report\MEng_Project_Report_template\figures\90degMUSICpostpilot','epsc')